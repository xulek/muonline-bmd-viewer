...
export function decryptModulusCryptor(source: Uint8Array): Uint8Array {
    ...
    logger.debug(`[ModulusCryptor] size=${size}, dataSize=${dataSize}, algo1=${algorithm1}(${CIPHER_NAMES[algorithm1 & 7]}), algo2=${algorithm2}(${CIPHER_NAMES[algorithm2 & 7]})`);
    ...
    logger.debug(`[ModulusCryptor] Stage1: cipher=${CIPHER_NAMES[algorithm1 & 7]}, blockSize=${blockSize}, cipherBlockSize=${cipher1.getBlockSize()}`);

    if (dataSize > 4 * blockSize) {
        const index = 2 + (dataSize >>> 1);
        logger.debug(`[ModulusCryptor] Stage1: middle block at index=${index}, len=${blockSize}`);
    ...
    if (dataSize > blockSize) {
        ...
        logger.debug(`[ModulusCryptor] Stage1: end block at index=${index}, len=${blockSize}`);
        ...
        // Start block
        index = 2;
        logger.debug(`[ModulusCryptor] Stage1: start block at index=${index}, len=${blockSize}`);
    ...
    logger.debug(`[ModulusCryptor] key2 (first 16): [${Array.from(key2.slice(0, 16)).map(b => b.toString(16).padStart(2, '0')).join(' ')}]`);
    ...
    logger.debug(`[ModulusCryptor] Stage2: cipher=${CIPHER_NAMES[algorithm2 & 7]}, decryptSize=${decryptSize}, cipherBlockSize=${cipher2.getBlockSize()}`);
    ...
    logger.debug(`[ModulusCryptor] Result first 10: [${Array.from(result.slice(0, 10)).join(', ')}]`);
